...
class unetUp(nn.Module):
    ...
    def forward(self, inputs0, *input):
        outputs0 = self.up(inputs0)  # 输入特征的c/2
        if input is not None:             
            for i in range(len(input)):
                outputs0 = torch.cat([outputs0,input[i]], 1) 
        return self.conv(outputs0)  # concat后的特征c/2


class unetUp_add(nn.Module):
    def __init__(self, in_size, out_size, is_deconv):
        super(unetUp_add, self).__init__()
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=4, stride=2, padding=1)
        else:
            self.up = nn.UpsamplingBilinear2d(scale_factor=2)

    def forward(self,outputs0, input):
        outputs0 = self.up(outputs0)
        if input is not None:
            outputs0 = outputs0 + input
        return outputs0


# 增加spp后的第一层unetup
class unetUp_cat(nn.Module):
    def __init__(self, in_size, out_size,cat_size, is_deconv, n_concat=2):
...
class resnet_unetUp(nn.Module):
    ...
    def forward(self, inputs0, inputskip):
        outputs0 = self.up(inputs0) # 先将输入上采样（h/w/c均变为原来的1/2)
        # print(outputs0.shape)
