...
2、int/double/long等都是java预定义的数据类型，Boolean、Character、Byte、Short、Integer、Long、Float、Double则是java.lang包（无需import）里定义的类，是Number的子类，包装了基本数据类型，包含多种功能。
...
3、判断String是否相等用String的内部方法`.equals()`。

4、判断语句中`?:`   `&&`   `||`前后的两个条件可以不加括号，因为与 或的优先级很低。
...
构造器又叫构造方法，是类的一种特殊成员方法。主要用于在创建新对象时，完成其**初始化**。构造器**没有返回值和返回类型**，**构造器的方法名和类名需要保持一致**。在对象创建过程中，系统会**自动**调用该类的构造器完成对象的初始化操作。
...
编译类型可以理解为编译器看到的对象的类型，运行类型可以理解为对象真正执行时的类型。**编译类型在对象定义时=号的左边，运行类型在=号的右边**；
