...
- `/查找词`+回车，输入n键切换下一个词（重新输入斜杠即可换查找词）
...
`ls [-options] path`	打印目录（`-l`以列表形式打印包括权限的详细信息 `-a`打印包括隐藏文件(.xxx)的信息 `-h`按最大的单位显示大小）

`ll path`	查看path目录的详细信息	（文件类型：“-”:普通文件	“d":目录	“l":链接)
...
`data`	显示当前时间	(`data +%D`	显示年月日	`+%Y`表示年 `+%m` 表示月 `+%d`表示日)
...
less快捷键：**空格或pagedown（向下一页）pageup(向上一页)  /字符串（查询[n向下 N向上]) ?字符串（向上搜寻【/相反】） q(返回)**
...
`find [range] -name filename`	按指定文件名查找)	
